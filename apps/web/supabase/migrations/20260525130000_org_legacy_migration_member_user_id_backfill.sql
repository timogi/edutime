-- Backfill organization_members.user_id from auth.users via ilike (legacy app behavior).
-- Safe to re-run: migrate_one_legacy_organization skips existing active org_seat entitlements.
...
begin
  ...
  return billing.backfill_organization_member_user_ids(v_ids);
end;
$$;
