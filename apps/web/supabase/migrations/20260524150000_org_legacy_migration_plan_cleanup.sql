-- Simplify billing.org_legacy_migration_plan and auto-populate seat_count.

alter table billing.org_legacy_migration_plan
  add column if not exists seat_count int null;

alter table billing.org_legacy_migration_plan
  add column if not exists annual_amount_cents int null;

update billing.org_legacy_migration_plan p
set
  seat_count = greatest(
    3,
    coalesce(p.custom_seat_count, 0),
    coalesce((select o.seats from public.organizations o where o.id = p.organization_id), 0),
    billing.count_org_legacy_seat_users(p.organization_id)
  ),
  annual_amount_cents = coalesce(p.annual_amount_cents, p.custom_annual_amount_cents);

alter table billing.org_legacy_migration_plan
  drop column if exists due_date,
  drop column if exists custom_seat_count,
  drop column if exists custom_annual_amount_cents,
  drop column if exists migrate_invites;

alter table billing.org_legacy_migration_plan
  drop constraint if exists org_legacy_migration_plan_custom_seat_count_check;

alter table billing.org_legacy_migration_plan
  drop constraint if exists org_legacy_migration_plan_custom_annual_amount_cents_check;

alter table billing.org_legacy_migration_plan
  add constraint org_legacy_migration_plan_seat_count_check
  check (seat_count is null or seat_count >= 3);

alter table billing.org_legacy_migration_plan
  add constraint org_legacy_migration_plan_annual_amount_cents_check
  check (annual_amount_cents is null or annual_amount_cents > 0);

comment on column billing.org_legacy_migration_plan.seat_count is
  'Licensed seats for this org. Filled by seed (max of org.seats, active members + admins, minimum 3). May be raised manually before migrate.';
comment on column billing.org_legacy_migration_plan.annual_amount_cents is
  'Total annual org license amount in minor units (e.g. Rappen). Required when migrate=true.';
comment on column billing.org_legacy_migration_plan.renewal_at is
  'Next renewal (subscriptions.current_period_end). Required when migrate=true.';
...
drop function if exists billing.resolve_org_migration_amount(int, int);
...
begin
  ...
  values (p_organization_id, billing.compute_org_legacy_seat_count(p_organization_id))
  on conflict (organization_id) do update
