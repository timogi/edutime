-- Free orgs: migrate without annual_amount_cents / renewal_at (defaults applied).

alter table billing.org_legacy_migration_plan
  add column if not exists is_free boolean not null default false;

comment on column billing.org_legacy_migration_plan.is_free is
  'When true: no payment required. annual_amount_cents defaults to 0, renewal_at defaults to now()+10 years if unset.';

alter table billing.org_legacy_migration_plan
  drop constraint if exists org_legacy_migration_plan_annual_amount_cents_check;
