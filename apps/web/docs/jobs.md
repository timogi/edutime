...
- **Current purpose:**
  ...
  - schedule **payment-notice** reminders (`billing.run_org_renewal_reminder_sweep`: on **invoice due date** and again **due + 45 days**) and send pending rows via Resend when `RESEND_API_KEY` is set (no separate Resend e-mails from the delinquency RPCs themselves)
  - soft delinquency (`billing.run_org_delinquency_sweep`): from **invoice due date** (calendar day): subscription metadata suspended, active `org_seat` entitlements **revoked** (`payment_failed`)
  - hard delinquency (`billing.run_org_hard_delinquency_sweep`): unpaid invoice past **due + 45 days** → deactivate org (45 days after due = 90 days after renewal when due is renewal+45)
  - purge admin-scheduled org deletions (`billing.purge_organizations_past_scheduled_deletion`): deletes organizations whose `scheduled_deletion_at` is set and **≤ today**, including related billing rows, entitlements, invites, members, admins, and legal org acceptances
...
|-----------|----------------|
| **Due date** | Same calendar day (job order): `run_org_renewal_reminder_sweep` may insert `payment_deadline_passed` → **org-billing-jobs** sends “Zahlungsfrist erreicht” if `RESEND_API_KEY` is set; then `run_org_delinquency_sweep`: subscription metadata `org_billing_status = suspended`, active `org_seat` entitlements **revoked** (`payment_failed`). |
| **Due + 45 days** | Same calendar day (job order): sweep may insert `pre_org_deactivation` → final notice e-mail; then `run_org_hard_delinquency_sweep`: `billing.deactivate_organization_for_nonpayment` — `organizations.is_active = false`, memberships/invites cleared, `org_seat` entitlements expired. |
